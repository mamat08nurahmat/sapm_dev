            ...
            <div class="row">
				...
				<script type="text/javascript">
						...
						if (com=='Delete')
							{
							   if($('.trSelected',grid).length>0 && $('.trSelected',grid).length<2) {
								...
								var items = $('.trSelected',grid);
									...
									konfirmasi = confirm('Apakah anda ingin menghapus user : '+nama+'?');
									
									if (konfirmasi){
										//var itemlist =items[0].id.substr(3);
										var urls = '<?php echo site_url("/phr_user/del_user")?>'+'/'+id;
						...
						if (com=='Edit')
							{
							   if($('.trSelected',grid).length>0 && $('.trSelected',grid).length<2) {
								...
								var items = $('.trSelected',grid);
								//var itemlist =items[0].id.substr(3);
								var urls = '<?php echo site_url("/phr_user/edt_user")?>'+'/'+id;
									window.location = urls ; 
								} else { 
									alert('Pilih satu data saja !'); 
								}
					...
					function add(com,grid)
					{
						if (com=='Add')
						{
							var urls = '<?php echo site_url("/phr_user/add_user")?>';
							window.location = urls; 
						}          
					}
				...
				?>
				 <div class="panel panel-default">	
					<div class="panel-heading" style="background-color:#8FC0D8; color:#fff;"><STRONG>LIST USER</STRONG></div>
					<?php echo $js_grid; ?>
